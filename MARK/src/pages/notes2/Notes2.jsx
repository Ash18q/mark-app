import React, { useState, useEffect, useMemo, useCallback } from 'react'
import { supabase } from '../../supabaseClient'
import { useAuth } from '../../context/AuthContext'
import NoteList2 from './NoteList2'
import NoteEditor2 from './NoteEditor2'
import MindMapModal2 from './MindMapModal2'

export default function Notes2({ isGhostMode = false }) {
  const auth = useAuth()
  const oldNotes = useMemo(() => Array.isArray(auth?.notes) ? auth.notes : [], [auth])
  const [notes2, setNotes2] = useState([])
  const [loading, setLoading] = useState(true)

  // Filters & UI States
  const [searchQuery, setSearchQuery] = useState('')
  const [selectedTag, setSelectedTag] = useState('')
  const [showArchived, setShowArchived] = useState(false)
  const [viewMode, setViewMode] = useState('grid') // 'grid' | 'list'

  // Modals
  const [editorState, setEditorState] = useState({ isOpen: false, note: null })
  const [mindMapNote, setMindMapNote] = useState(null)

  // Fetch / Sync notes2 from Supabase with LocalStorage & oldNotes fallback
  const fetchNotes2 = useCallback(async () => {
    setLoading(true)
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      const userId = sessionData?.session?.user?.id

      if (!userId) {
        setNotes2(oldNotes)
        setLoading(false)
        return
      }

      // Try fetching from notes2 table
      const { data, error } = await supabase
        .from('notes2')
        .select('*')
        .eq('user_id', userId)
        .order('is_pinned', { ascending: false })
        .order('updated_at', { ascending: false })

      if (!error && Array.isArray(data) && data.length > 0) {
        setNotes2(data)
        try { localStorage.setItem(`mark_notes2_${userId}`, JSON.stringify(data)) } catch { /* ignore */ }
        setLoading(false)
        return
      } else if (oldNotes && oldNotes.length > 0) {
        // If notes2 table is empty, auto-sync existing notes from notes table into notes2
        console.log('[MARK] Syncing existing notes into notes2 table...')
        const synced = oldNotes.map(n => ({
          title: n.title || 'Untitled Note',
          content: n.content || '',
          tags: n.tags ? (typeof n.tags === 'string' ? n.tags.split(',').map(t => t.trim()) : n.tags) : [],
          color: n.color || '#FFFFFF',
          is_pinned: Boolean(n.is_pinned),
          is_archived: Boolean(n.is_archived),
          user_id: userId,
          created_at: n.created_at || new Date().toISOString(),
          updated_at: n.updated_at || new Date().toISOString()
        }))

        try {
          const { data: inserted, error: insertErr } = await supabase
            .from('notes2')
            .insert(synced)
            .select()

          if (!insertErr && Array.isArray(inserted) && inserted.length > 0) {
            setNotes2(inserted)
            try { localStorage.setItem(`mark_notes2_${userId}`, JSON.stringify(inserted)) } catch { /* ignore */ }
            setLoading(false)
            return
          }
        } catch (e) {
          console.warn('[MARK] Supabase notes2 sync error:', e)
        }
      }
    } catch (err) {
      console.warn('[MARK] Supabase notes2 fetch error, falling back:', err)
    }

    // LocalStorage Fallback if notes2 table missing or offline
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      const userId = sessionData?.session?.user?.id
      if (userId) {
        const local = localStorage.getItem(`mark_notes2_${userId}`)
        if (local) {
          setNotes2(JSON.parse(local))
          setLoading(false)
          return
        }
      }
    } catch { /* ignore */ }

    // Fallback to oldNotes if nothing else
    setNotes2(oldNotes)
    setLoading(false)
  }, [oldNotes])

  useEffect(() => {
    fetchNotes2()
  }, [fetchNotes2])

  // Save / Update Note2
  const handleSaveNote2 = async (notePayload) => {
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      const userId = sessionData?.session?.user?.id

      if (!userId) return

      let savedNote = { ...notePayload, user_id: userId }

      if (notePayload.id) {
        // Update existing
        try {
          const { data, error } = await supabase
            .from('notes2')
            .update({
              title: notePayload.title,
              content: notePayload.content,
              tags: notePayload.tags,
              color: notePayload.color,
              is_pinned: notePayload.is_pinned,
              is_archived: notePayload.is_archived,
              updated_at: new Date().toISOString()
            })
            .eq('id', notePayload.id)
            .select()
            .single()

          if (!error && data) savedNote = data
        } catch (e) {
          console.warn('[MARK] notes2 update error:', e)
        }

        setNotes2(prev => {
          const updated = prev.map(n => n.id === notePayload.id ? { ...n, ...savedNote } : n)
          try { localStorage.setItem(`mark_notes2_${userId}`, JSON.stringify(updated)) } catch { /* ignore */ }
          return updated
        })
      } else {
        // Create new
        savedNote.id = 'note2_' + Date.now() + '_' + Math.random().toString(36).substr(2, 4)
        try {
          const { data, error } = await supabase
            .from('notes2')
            .insert([{
              title: notePayload.title,
              content: notePayload.content,
              tags: notePayload.tags,
              color: notePayload.color,
              is_pinned: notePayload.is_pinned,
              is_archived: notePayload.is_archived,
              user_id: userId
            }])
            .select()
            .single()

          if (!error && data) savedNote = data
        } catch (e) {
          console.warn('[MARK] notes2 insert error:', e)
        }

        setNotes2(prev => {
          const updated = [savedNote, ...prev]
          try { localStorage.setItem(`mark_notes2_${userId}`, JSON.stringify(updated)) } catch { /* ignore */ }
          return updated
        })
      }
    } catch (e) {
      console.error('[MARK] handleSaveNote2 error:', e)
    }

    setEditorState({ isOpen: false, note: null })
  }

  // Delete Note2
  const handleDeleteNote2 = async (id) => {
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      const userId = sessionData?.session?.user?.id

      try {
        await supabase.from('notes2').delete().eq('id', id)
      } catch (e) {
        console.warn('[MARK] notes2 delete error:', e)
      }

      setNotes2(prev => {
        const updated = prev.filter(n => n.id !== id)
        if (userId) {
          try { localStorage.setItem(`mark_notes2_${userId}`, JSON.stringify(updated)) } catch { /* ignore */ }
        }
        return updated
      })
    } catch (e) {
      console.error('[MARK] handleDeleteNote2 error:', e)
    }

    setEditorState({ isOpen: false, note: null })
  }

  // Toggle Pin Note2
  const handleTogglePinNote2 = async (id) => {
    const target = notes2.find(n => n.id === id)
    if (target) {
      await handleSaveNote2({ ...target, is_pinned: !target.is_pinned })
    }
  }

  // Toggle Archive Note2
  const handleToggleArchiveNote2 = async (id) => {
    const target = notes2.find(n => n.id === id)
    if (target) {
      await handleSaveNote2({ ...target, is_archived: !target.is_archived })
    }
  }

  // Derive All Available Tags
  const allTags = useMemo(() => {
    const tagsSet = new Set()
    if (Array.isArray(notes2)) {
      notes2.forEach(n => {
        if (!n) return
        if (Array.isArray(n.tags)) {
          n.tags.forEach(t => t && tagsSet.add(t))
        } else if (typeof n.tags === 'string') {
          n.tags.split(',').forEach(t => t && tagsSet.add(t.trim()))
        }
      })
    }
    return [...tagsSet].filter(Boolean)
  }, [notes2])

  // Filtered Notes2
  const filteredNotes = useMemo(() => {
    if (!Array.isArray(notes2)) return []
    return notes2.filter(n => {
      if (!n) return false

      // Archive filter
      if (showArchived ? !n.is_archived : n.is_archived) return false

      // Tag filter
      if (selectedTag) {
        const noteTags = Array.isArray(n.tags)
          ? n.tags
          : typeof n.tags === 'string'
            ? n.tags.split(',').map(t => t.trim())
            : []
        if (!noteTags.map(t => t.toLowerCase()).includes(selectedTag.toLowerCase())) return false
      }

      // Search query
      if (searchQuery.trim()) {
        const q = searchQuery.toLowerCase().trim()
        const titleMatch = (n.title || '').toLowerCase().includes(q)
        const contentMatch = (n.content || '').toLowerCase().includes(q)
        const tagsMatch = Array.isArray(n.tags) ? n.tags.some(t => (t || '').toLowerCase().includes(q)) : (n.tags || '').toLowerCase().includes(q)
        if (!titleMatch && !contentMatch && !tagsMatch) return false
      }

      return true
    })
  }, [notes2, showArchived, selectedTag, searchQuery])

  return (
    <div className="space-y-5 pb-20">
      
      {/* ── Header Title Row ── */}
      <div className="flex items-center justify-between bg-white border border-slate-100/90 rounded-2xl px-4 py-3.5 shadow-sm">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-600 to-purple-600 text-white flex items-center justify-center font-bold text-base shadow-sm">
            🗒️
          </div>
          <div>
            <h2 className="text-slate-900 font-extrabold text-base tracking-tight leading-tight flex items-center gap-2">
              <span>Notes 2</span>
              <span className="text-[10px] bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-2 py-0.5 rounded-full font-extrabold shadow-2xs">
                PRO
              </span>
            </h2>
            <p className="text-slate-400 text-[10px] font-medium">{filteredNotes.length} notes stored</p>
          </div>
        </div>

        {/* Top Right Create Note Button */}
        <button
          type="button"
          onClick={() => setEditorState({ isOpen: true, note: null })}
          className="px-3.5 py-2 bg-gradient-to-tr from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-extrabold text-xs rounded-xl shadow-md transition cursor-pointer flex items-center gap-1.5 active:scale-95"
        >
          <span>➕</span>
          <span>New Note</span>
        </button>
      </div>

      {/* ── Search & Filter Control Bar ── */}
      <div className="bg-white border border-slate-100 rounded-2xl p-3 shadow-xs space-y-3">
        {/* Search Bar */}
        <div className="relative">
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search notes by title, content, or tag..."
            className="w-full bg-slate-50 border border-slate-200/80 rounded-xl px-3.5 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-100 font-medium pr-8"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => setSearchQuery('')}
              className="absolute right-2.5 top-2 text-slate-400 hover:text-slate-600 text-xs font-bold"
            >
              ✕
            </button>
          )}
        </div>

        {/* Controls Row: Tag Chips + View Mode + Archive Toggle */}
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs pt-1 border-t border-slate-100">
          {/* Tag Filter Pills */}
          <div className="flex items-center gap-1.5 overflow-x-auto py-0.5 flex-1 min-w-[200px]">
            <button
              type="button"
              onClick={() => setSelectedTag('')}
              className={`px-3 py-1 rounded-full font-bold text-[11px] transition cursor-pointer shrink-0 ${
                selectedTag === ''
                  ? 'bg-indigo-600 text-white shadow-2xs'
                  : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
            >
              All ({notes2.length})
            </button>
            {allTags.map(tag => (
              <button
                key={tag}
                type="button"
                onClick={() => setSelectedTag(selectedTag === tag ? '' : tag)}
                className={`px-3 py-1 rounded-full font-bold text-[11px] transition cursor-pointer shrink-0 ${
                  selectedTag === tag
                    ? 'bg-indigo-600 text-white shadow-2xs'
                    : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
              >
                #{tag}
              </button>
            ))}
          </div>

          {/* Right View Mode & Archive Toggles */}
          <div className="flex items-center gap-2 shrink-0">
            {/* Archive Toggle */}
            <button
              type="button"
              onClick={() => setShowArchived(!showArchived)}
              className={`px-2.5 py-1 rounded-xl text-[11px] font-bold border transition cursor-pointer ${
                showArchived
                  ? 'bg-purple-100 border-purple-200 text-purple-700'
                  : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
              }`}
            >
              📦 {showArchived ? 'Archived' : 'Active'}
            </button>

            {/* View Mode Grid/List */}
            <div className="flex items-center bg-slate-100 p-0.5 rounded-xl border border-slate-200/60">
              <button
                type="button"
                onClick={() => setViewMode('grid')}
                className={`p-1 rounded-lg transition ${viewMode === 'grid' ? 'bg-white text-indigo-600 shadow-2xs' : 'text-slate-400 hover:text-slate-600'}`}
                title="Grid View"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                </svg>
              </button>
              <button
                type="button"
                onClick={() => setViewMode('list')}
                className={`p-1 rounded-lg transition ${viewMode === 'list' ? 'bg-white text-indigo-600 shadow-2xs' : 'text-slate-400 hover:text-slate-600'}`}
                title="List View"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
                </svg>
              </button>
            </div>
          </div>
        </div>
      </div>

      {/* ── Main Note List Grid ── */}
      {loading ? (
        <div className="py-12 text-center text-slate-400 text-xs font-medium">
          Loading Notes 2...
        </div>
      ) : (
        <NoteList2
          notes={filteredNotes}
          viewMode={viewMode}
          onEdit={(note) => setEditorState({ isOpen: true, note })}
          onDelete={handleDeleteNote2}
          onTogglePin={handleTogglePinNote2}
          onToggleArchive={handleToggleArchiveNote2}
          onOpenMindMap={(note) => setMindMapNote(note)}
          isGhostMode={isGhostMode}
        />
      )}

      {/* ── Note Editor Modal ── */}
      {editorState.isOpen && (
        <NoteEditor2
          note={editorState.note}
          onSave={handleSaveNote2}
          onClose={() => setEditorState({ isOpen: false, note: null })}
          onDelete={handleDeleteNote2}
        />
      )}

      {/* ── Mind Map Modal ── */}
      {mindMapNote && (
        <MindMapModal2
          note={mindMapNote}
          onClose={() => setMindMapNote(null)}
        />
      )}
    </div>
  )
}
