...
import { PASTEL_COLORS } from './NoteCard2'

export default function NoteEditor2({ note, onSave, onClose, onDelete }) {
  ...
  const insertMarkdown = (prefix, suffix = '') => {
    const textarea = document.getElementById('note-editor-content-2')
    if (!textarea) return
  ...
  const selectedColorObj = PASTEL_COLORS.find(c => c.hex.toLowerCase() === color.toLowerCase()) || PASTEL_COLORS[0]
      ...
      <div className={`relative w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden border flex flex-col max-h-[90vh] transition-colors duration-200 ${selectedColorObj.bg} ${selectedColorObj.border} ${selectedColorObj.text}`}>
        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1">
          ...
          <input
            ...
            className={`w-full bg-transparent font-black text-lg focus:outline-none placeholder:text-slate-400 ${isDark ? 'text-white' : 'text-slate-900'}`}
          />
          ...
          <div className={`p-2 rounded-2xl border flex flex-wrap items-center gap-1 text-xs ${isDark ? 'bg-slate-800/80 border-slate-700' : 'bg-white/80 border-slate-200/80'}`}>
            ...
            <button type="button" onClick={() => insertMarkdown('> ')} className="px-2 py-1 hover:bg-black/5 rounded-lg">Quote</button>
          ...
          {editorTab === 'write' ? (
            ...
            <div className={`w-full min-h-[160px] p-3 rounded-2xl border text-sm leading-relaxed whitespace-pre-wrap font-medium ${isDark ? 'bg-slate-800/50 border-slate-700 text-slate-200' : 'bg-white/50 border-slate-200 text-slate-800'}`}>
          ...
          <div className={`p-4 rounded-2xl border ${isDark ? 'bg-slate-800/40 border-slate-700' : 'bg-white/60 border-slate-200/80'}`}>
            <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <span>☑️ Interactive Checklist Items</span>
            </h4>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-3 py-2 bg-indigo-600 text-white font-bold text-xs rounded-xl shadow-2xs hover:bg-indigo-700 transition cursor-pointer"
              >
                + Add
              </button>
            </div>
        ...
        <div className={`p-4 px-6 border-t flex items-center justify-between ${isDark ? 'border-slate-800' : 'border-black/5'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={handleSave}
              className="px-6 py-2.5 bg-gradient-to-tr from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold text-xs rounded-xl shadow-md transition cursor-pointer"
            >
              Save Note
            </button>
          </div>
