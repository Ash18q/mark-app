...
const DEFAULT_PLATFORMS = [
  'YouTube', 'Instagram', 'Pinterest', 'Threads', 'Rednote', 'Snapchat', 'Telegram',
  'Facebook', 'Twitter/X', 'LinkedIn', 'GitHub', 'Reddit', 'Discord',
  'TikTok', 'Spotify', 'Medium', 'Twitch', 'Notion'
]
...
const PinterestTileIcon = ({ className = "w-4 h-4" }) => (
  <img src="/pinterest.png" alt="Pinterest" className={`${className} object-contain inline-block rounded-xs`} />
)
const ThreadsTileIcon = ({ className = "w-4 h-4" }) => (
  <img src="/threads.png" alt="Threads" className={`${className} object-contain inline-block rounded-xs`} />
)
...
const SnapchatTileIcon = ({ className = "w-4 h-4" }) => (
  <img src="/snapchat.png" alt="Snapchat" className={`${className} object-contain inline-block rounded-xs`} />
)
...
function getPlatformTileIcon(p, className = "w-4 h-4") {
  ...
  if (name.includes('pinterest') || name === 'pin') return <PinterestTileIcon className={className} />
...
function detectPlatform(url) {
  ...
  try {
    ...
    if (host.includes('xiaohongshu.com') || host.includes('rednote')) return 'Rednote'
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between bg-white border border-slate-100/90 rounded-2xl px-4 py-3 shadow-sm">
          ...
          {/* Top Right 3 Action Icons: Search, Filter, Export (Clear Contrast Purple Pills) */}
          <div className="flex items-center gap-2">
            {/* 1. Search Icon */}
            <button
              type="button"
              onClick={() => setIsSearchOpen(!isSearchOpen)}
              className={`w-9 h-9 sm:w-10 sm:h-10 rounded-2xl flex items-center justify-center transition cursor-pointer active:scale-95 shadow-2xs ${
                isSearchOpen || searchQuery
                  ? 'bg-purple-600 text-white font-bold shadow-md shadow-purple-500/20'
                  : 'bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-150/80'
              }`}
              title="Search links"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              className={`w-9 h-9 sm:w-10 sm:h-10 rounded-2xl flex items-center justify-center transition relative cursor-pointer active:scale-95 shadow-2xs ${
                hasActiveFilters
                  ? 'bg-purple-600 text-white font-bold shadow-md shadow-purple-500/20'
                  : 'bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-150/80'
              }`}
            ...
            >
              ...
              {hasActiveFilters && (
                <span className="w-2.5 h-2.5 bg-amber-400 rounded-full absolute top-1 right-1 ring-2 ring-white" />
              )}
            </button>
            ...
            <button
              ...
              className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-150/80 flex items-center justify-center transition cursor-pointer active:scale-95 shadow-2xs"
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 font-sans">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 pb-28">
        ...
        {activeTab === 'jot' && <JotMain isGhostMode={isGhostMode} />}
        ...
        <nav className="max-w-lg mx-auto bg-white border border-slate-100 rounded-3xl shadow-xl shadow-slate-200/60 px-1.5 py-2 pointer-events-auto flex items-center justify-around">
          ...
          >
            <div className={`px-3 sm:px-4 py-1 rounded-full transition-all duration-200 flex items-center justify-center ${
              ...
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={activeTab === 'jot' ? 2.5 : 2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
            </div>
            <span className={`text-[9px] sm:text-[11px] transition-colors leading-tight ${
              activeTab === 'jot' ? 'text-indigo-600 font-bold' : 'text-slate-500 font-medium group-hover:text-slate-800'
            }`}>
              Jot Notes
            </span>
          </button>
