...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 font-sans">
      ...
      {/* ── Floating Bottom Navigation Bar (5 Sections) ── */}
      <div className="fixed bottom-4 left-0 right-0 z-40 px-2 sm:px-3 pointer-events-none">
        <nav className="max-w-md mx-auto bg-white border border-slate-100 rounded-3xl shadow-xl shadow-slate-200/60 p-1.5 sm:p-2 pointer-events-auto flex items-center justify-between gap-0.5 sm:gap-1">
          {/* Section 1: Add Link */}
          <button
            id="tab-add"
            onClick={() => setActiveTab('add')}
            className={`px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-2xl flex items-center gap-1.5 text-[11px] sm:text-xs transition-all duration-150 cursor-pointer ${
              activeTab === 'add'
          ...
          <button
            ...
            className={`px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-2xl flex items-center gap-1 text-[11px] sm:text-xs transition-all duration-150 cursor-pointer ${
              activeTab === 'library'
                ? 'bg-slate-100 text-slate-900 font-bold shadow-2xs'
                : 'text-slate-600 hover:text-slate-900 font-medium'
            }`}
          >
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
